#!/bin/bash
# Get Lark Minutes info by minute_token
# Usage: get_minutes.sh <minute_token> [user_id_type]
# minute_token: 24-char token from minutes URL (last segment)
...
curl -s -X GET \
  "https://open.larksuite.com/open-apis/minutes/v1/minutes/${MINUTE_TOKEN}?user_id_type=${USER_ID_TYPE}" \
